...
A negative binary value is expressed in **two's complement notation**. According to this rule, to convert a binary number to its negative value is to _reverse its bit values and add 1_.
...
To subtract one value from another, _convert the number being subtracted to two's complement format and add the numbers_.
...
The processor may access one or more bytes of memory at a time. Let us consider a hexadecimal number 0725. This number will require two bytes of memory. The high-order byte or most significant byte is 07 and the low-order byte is 25.
