[Sashko Stubailo](https://www.apollographql.com/blog/author/sashko)
...
_Note: In the example above, some REST APIs would return “author” as a separate resource._
...
There are a few interesting bits here when compared to the REST routes for a similar data set. First, instead of sending a different HTTP verb to the same URL to differentiate a read vs. a write, GraphQL uses a different _initial type_ — Mutation vs. Query. In a GraphQL document, you can select which type of operation you’re sending with a keyword:
...
_For all of the details about the query language, read my earlier post,_ [_“The Anatomy of a GraphQL Query”_](https://dev-blog.apollodata.com/the-anatomy-of-a-graphql-query-6dffa9e9e747)_._
...
On the other hand, GraphQL doesn’t have as many tools and integrations as REST yet. For example, you can’t cache GraphQL results using HTTP caching as effectively as you can with REST results. However, there are tools that help remedy this. For example, you can cache GraphQL results in your frontend using [Apollo Client](http://www.apollographql.com/client), [urql](https://formidable.com/open-source/urql/), and [Relay](https://facebook.github.io/relay/). On the server side, [RESTDataSource](https://www.apollographql.com/docs/apollo-server/data/data-sources/#rest-data-source) can help with caching GraphQL results fetched from REST endpoints.
